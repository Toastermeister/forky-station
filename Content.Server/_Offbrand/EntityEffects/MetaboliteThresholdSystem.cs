...
public sealed partial class MetaboliteThresholdEntityConditionSystem : EntityConditionSystem<MetabolizerComponent, MetaboliteThresholdCondition>
{
    ...
    protected override void Condition(Entity<MetabolizerComponent> ent, ref EntityConditionEvent<MetaboliteThresholdCondition> args)
    {
        ...
        if (!_metabolizer.TryGetMetabolitesSolution(ent, stage, out var metabolitesSolution, out _, out _))
            return;

        var quant = metabolitesSolution.GetTotalPrototypeQuantity(args.Condition.Reagent);

        if (args.Condition.IncludeBloodstream && solutions.TryGetValue(BloodstreamStage, out var bloodstreamStage) && _metabolizer.TryGetMetabolitesSolution(ent, bloodstreamStage, out var solution, out _, out _))
        {
