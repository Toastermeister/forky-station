...
public sealed partial class SurgeryToolSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<SurgeryToolComponent, ToolUserAttemptUseEvent>(OnToolAttemptUse);
    }

    private void OnToolAttemptUse(Entity<SurgeryToolComponent> ent, ref ToolUserAttemptUseEvent args)
    {
        ...
        if (_inventory.TryGetContainerSlotEnumerator(target, out var enumerator, ent.Comp.SlotsToCheck))
        {
            while (enumerator.MoveNext(out var slot))
            {
                ...
                _popup.PopupCursor(Loc.GetString(ent.Comp.SlotsDenialPopup, ("target", args.Target), ("clothing", contained)), ent);
                args.Cancelled = true;
                return;
            }
        }
        ...
        {
            _popup.PopupCursor(Loc.GetString(ent.Comp.DownDenialPopup, ("target", args.Target)), ent);
            args.Cancelled = true;
