...
public sealed partial class WoundableBodySystem : OffbrandDamageSystem
{
    ...
    private void OnDamageDealt(Entity<WoundableBodyComponent> ent, ref DamageDealtEvent args)
    {
        ...
        if (delta.AnyPositive())
        {
            ...
            var organEvt = new DamageDealtEvent(DamageSpecifier.GetPositive(delta), args.Origin, args.InterruptsDoAfters);
            RaiseLocalEvent(target, ref organEvt);
        }

        var negative = DamageSpecifier.GetNegative(delta);
        if (!negative.Empty)
            HealWounds(ent, negative, false, false);
