...
public sealed partial class CrewMonitoringWindow : FancyWindow
{
    ...
    public void ShowSensors(List<PDASensorStatus> sensors, EntityUid monitor, EntityCoordinates? monitorCoords)
    {
        ClearOutDatedData();
        ...
        Dictionary<NetEntity, PDASensorStatus> uniqueSensorsMap = new();
        ...
        var assignedSensors = new HashSet<PDASensorStatus>();
    ...
    private void PopulateDepartmentList(IEnumerable<PDASensorStatus> departmentSensors)
    {
        ...
        foreach (var sensor in departmentSensors)
        {
            ...
            NavMap.LocalizedNames.TryAdd(sensor.PDASensorUid, sensor.Name + ", " + sensor.Job);

            var sensorButton = new CrewMonitoringButton()
            {
                PDASensorUid = sensor.PDASensorUid,
                Coordinates = coordinates,
                Disabled = (coordinates == null),
                HorizontalExpand = true,
            };

            if (sensor.PDASensorUid == _trackedEntity)
            ...
            // PDA coords indicator
            var PDACoordsIndicator = new TextureRect()
            {
            ...
            statusContainer.AddChild(PDACoordsIndicator);
            ...
            {
                NavMap.TrackedEntities.TryAdd(sensor.PDASensorUid,
                    new NavMapBlip
                    (CoordinatesToLocal(coordinates.Value),
                    _blipTexture,
                    (_trackedEntity == null || sensor.PDASensorUid == _trackedEntity) ? Color.LimeGreen : Color.LimeGreen * Color.DimGray,
                    sensor.PDASensorUid == _trackedEntity));
                ...
                sensorButton.OnButtonUp += args =>
                {
                    ...
                    if (_trackedEntity == sensor.PDASensorUid)
                    {
                        _trackedEntity = null;
                    }
                    ...
                    {
                        _trackedEntity = sensor.PDASensorUid;
    ...
    private void UpdateSensorsTable(NetEntity? currTrackedEntity, NetEntity? prevTrackedEntity)
    {
        foreach (var sensor in SensorsTable.Children)
        {
            ...
            if (castSensor.PDASensorUid == prevTrackedEntity)
                castSensor.RemoveStyleClass(StyleClass.Positive);

            else if (castSensor.PDASensorUid == currTrackedEntity)
                castSensor.AddStyleClass(StyleClass.Positive);

            if (castSensor?.Coordinates == null)
                continue;

            if (NavMap.TrackedEntities.TryGetValue(castSensor.PDASensorUid, out var data))
            {
                data = new NavMapBlip
                    (CoordinatesToLocal(data.Coordinates),
                    data.Texture,
                    (currTrackedEntity == null || castSensor.PDASensorUid == currTrackedEntity) ? Color.LimeGreen : Color.LimeGreen * Color.DimGray,
                    castSensor.PDASensorUid == currTrackedEntity);

                NavMap.TrackedEntities[castSensor.PDASensorUid] = data;
            }
        }
    }
    ...
    private bool TryGetNextScrollPosition([NotNullWhen(true)] out float? nextScrollPosition)
    {
        ...
        foreach (var sensor in SensorsTable.Children)
        {
            if (sensor is CrewMonitoringButton &&
                ((CrewMonitoringButton) sensor).PDASensorUid == _trackedEntity)
                return true;
